...
def ver_perfil(request):
    user = request.user
    media = user.media.all()  # Traemos todos los medios del usuario
    context = {
        'user': user,
        'media': media
    }
...
def editar_perfil(request):
    ...
    if request.method == 'POST':
        ...
        perfil_form = perfil_form_class(request.POST, request.FILES, instance=perfil_instance)
        media_form = ProfileMediaForm(request.POST, request.FILES)

        if user_form.is_valid() and perfil_form.is_valid() and media_form.is_valid():
            user_form.save()
